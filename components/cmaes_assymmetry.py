...
for coeff_set in coefficient_sets:
    ...
    for seed in seed_list:
        ...
        def modify_5bar_xml(
            ...
            # -----------------------------
            thigh_left_mass = float(calf_interp_func(l1))
            calf_left_mass = float(calf_interp_func(l2))
            thigh_right_mass = float(calf_interp_func(l3))
            calf_right_mass = float(calf_interp_func(l4))
            ...
            for geom in root.findall(".//geom[@name='thigh_left']"):
                ...
                geom.set("mass", str(thigh_left_mass))

            for geom in root.findall(".//geom[@name='thigh_right']"):
                geom.set("fromto", f"0 0 0 {l3} 0 0")
                geom.set("mass", str(thigh_right_mass))
            ...
            for geom in root.findall(".//geom[@name='shank_left']"):
                ...
                geom.set("mass", str(calf_left_mass))

            for geom in root.findall(".//geom[@name='shank_right']"):
                geom.set("fromto", f"0 0 0 {l4} 0 0")
                geom.set("mass", str(calf_right_mass))
        ...
        def get_cost(params):
            ...
            try:
                ...
                thigh_left_length = params[0]
                calf_left_length = params[1]
                thigh_right_length = params[2]
                calf_right_length = params[3]
                torso_distance = params[4]
                ik_height = params[5]
                max_reach_left = thigh_left_length + calf_left_length
                max_reach_right = thigh_right_length + calf_right_length
                max_reach = min(max_reach_left, max_reach_right) 
                min_left_reach = abs(thigh_left_length - calf_left_length)
                min_right_reach = abs(thigh_right_length - calf_right_length)
                min_reach = max(min_left_reach, min_right_reach)
                ...
                with open(all_samples_file, "a", newline="") as file:
                    ...
                    for r in run_results:
                        ...
                        writer.writerow([
                            thigh_left_length, calf_left_length,
                            thigh_right_length, calf_right_length,
        ...
        # CMA-ES Optimization
        x0 = normalize(np.array([0.25, 0.25, 0.25, 0.25, 0.1, 0.35, 3, 4, 10.0, 10.0, 550, 5, 30]))
        sigma0 = 0.1
        opts = cma.CMAOptions()
        opts.set({
        ...
        with open(best_results_file, "a", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(["thigh_left_length", "calf_left_length","thigh_right_length", "calf_right_length", "motor_left_name", "motor_right_name", "gear_left_ratio", "gear_right_ratio", "gearbox_left", "gearbox_right", "torso_distance","ik_height", "best_index", "best_cost"] + [f"ac{i+1}" for i in range(3)])


        with open(all_samples_file, "a", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(["Thigh_left_length", "Calf_left_length", "Thigh_right_length", "Calf_right_length", "Hip left motor", "Hip right motor", "Hip left ratio", "Hip right ratio", "Gearbox left", "Gearbox right", "Efficiency left", "Efficiency right", "Torso distance", "ik_height", "Best X velocity", "Average energy", "Max height", "Max distance", "Unique id", "Cost"] + [f"ac{i+1}" for i in range(3)])
        ...
        if __name__ == "__main__":
            ...
            while not es.stop():
                ...
                thigh_left_length = best_params[0]
                calf_left_length = best_params[1]
                thigh_right_length = best_params[2]
                calf_right_length = best_params[3]
                torso_distance = best_params[4]
                ik_height = best_params[5]
