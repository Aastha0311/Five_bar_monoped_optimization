...
coefficient_sets = []
for first_coeff in np.arange(0.60, 0.69, 0.01):  # 0.4 to 0.8 with step 0.05
...
for coeff_set in coefficient_sets:
    ...
    for seed in seed_list:
        ...
        best_results_file = f"/home/stochlab/repo/optimal-design-legged-robots/results/planar/dist/doggo/25/best_dist_25_{coeff_str}_{date_str}_{seed}.csv"
        all_samples_file = f"/home/stochlab/repo/optimal-design-legged-robots/results/planar/dist/doggo/25/all_dist_25_{coeff_str}_{date_str}_{seed}.csv"
        ...
        original_bounds = np.array([
            ...
            [50, 10000],    # Controller Param 1
            [0, 10], 
            [10, 500]# Controller Param        # Controller Param 6
        ])
        import pandas as pd
        ...
        def get_cost(params):
            ...
            try:
                ...
                controller_params = process_action(params[0:])
                ...
                modified_xml = f"/home/stochlab/repo/optimal-design-legged-robots/xmls/doggo_xmls/{unique_id}.xml"
        ...
        # CMA-ES Optimization
        x0 = normalize(np.array([5500, 5, 300]))
        sigma0 = 0.1
        opts = cma.CMAOptions()
        opts.set({
            'maxiter': 1000, 'popsize': 8, 'seed': int(seed),
            'bounds': [np.zeros(3), np.ones(3)], 'verb_disp': 1000, 'verb_disp': 1})
            # 'tolfun': 0,
            # 'tolfunhist': 0,
            # 'tolx': 0,
            # 'tolstagnation': 1000,
        ...
        with open(best_results_file, "a", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(["thigh_length", "calf_length",  "best_index", "best_cost"] + [f"ac{i+1}" for i in range(3)])


        with open(all_samples_file, "a", newline="") as file:
            writer = csv.writer(file)
            writer.writerow(["Thigh", "Calf", "Best X velocity", "Average energy", "Max height", "Max distance", "Unique id", "Cost"] + [f"ac{i+1}" for i in range(3)])
        ...
        if __name__ == "__main__":
            ...
            while not es.stop():
                ...
                controller_params = process_action(best_params[0:])
