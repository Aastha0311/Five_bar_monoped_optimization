...
for coeff_set in coefficient_sets:
    ...
    for seed in seed_list:
        ...
        xml_template = os.path.join(XMLS_DIR, "5bar_baseline.xml")
          
        date_str = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        
        # Update filenames to include coefficient values
        output_dir = os.path.join(RESULTS_DIR, "planar", "gear_001_35gr")
        best_results_file = os.path.join(
            output_dir,
            f"best_gear_20_{coeff_str}_{date_str}_{seed}.csv",
        )
        all_samples_file = os.path.join(
            output_dir,
            f"all_gear_20_{coeff_str}_{date_str}_{seed}.csv",
        )
        ...
        def get_cost(params):
            ...
            try:
                ...
                modified_xml = os.path.join(XMLS_DIR, "gear_xmls", f"{unique_id}.xml")
